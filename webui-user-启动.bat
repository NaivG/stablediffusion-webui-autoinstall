...
if "%lng%"=="cn" (
    echo %GN%[INFO] %WT% ÊÇ·ñÆôÓÃpython venv£¿[Y/N]
  ) else (
...
if "%lng%"=="cn" (
    ...
    echo       NVIDIA£¨CUDA11£©Ñ¡Ôña£¬AMDÑ¡Ôñb£¬CPUÑ¡Ôñc
  ) else (
    ...
    choice -n -c abc >nul
        if errorlevel == 3 (
          if "%lng%"=="cn" echo %GN%[INFO] %WT% ÒÑÑ¡ÔñCPU°æ±¾¡£
          if "%lng%"=="en" echo %GN%[INFO] %WT% Choosed CPU mode.
          set TORCHVER=CPU
		  goto :choosenext
        )
        if errorlevel == 2 (
          if "%lng%"=="cn" echo %GN%[INFO] %WT% ÒÑÑ¡ÔñAMD°æ±¾¡£
          if "%lng%"=="cn" echo %GN%[WARN] %WT% ROCMÔÚWindowsÉÏ²»¿ÉÓÃ£¡
        ...
        if errorlevel == 1 (
          if "%lng%"=="cn" echo %GN%[INFO] %WT% ÒÑÑ¡ÔñNVIDIA£¨CUDA£©°æ±¾¡£
          if "%lng%"=="cn" echo %GN%[WARN] %WT% ÇëÈ·±£ÒÑ¾­ÌáÇ°°²×°ºÃCUDA»·¾³£¡
...
:torchnext
echo %GN%[INFO] %WT% %installtext% Ô­°æÒÀÀµ...
...
if "%lng%"=="cn" (
    ...
    choice -n -c abcd >nul
        ...
        if errorlevel == 3 (
          echo %GN%[INFO] %WT% ÒÑÑ¡ÔñµÍÅäÏÔ¿¨£¨6G¼°ÒÔÏÂÏÔ´æ£©¡£
          set method=3
          goto :argsnext
 )
        if errorlevel == 2 (
